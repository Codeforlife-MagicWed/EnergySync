...
export const RescuePreviewModal: React.FC<RescuePreviewModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl bg-white rounded-2xl border-[3px] border-slate-900 shadow-[6px_6px_0px_#0f172a] p-6 sm:p-8 space-y-6 text-left max-h-[90vh] flex flex-col justify-between">
        {/* Top Header */}
        <div className="flex items-start justify-between gap-4 border-b-2 border-slate-900 pb-4 flex-shrink-0">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="label-mono text-[11px] font-bold text-slate-700 flex items-center gap-1">
          ...
          <button
            ...
            className="p-2 rounded-xl border-2 border-slate-900 bg-white hover:bg-slate-100 text-slate-900 transition-transform active:translate-x-0.5 active:translate-y-0.5 cursor-pointer"
          >
        ...
        <div className="p-4 rounded-xl bg-amber-50 border-2 border-slate-900 text-xs text-slate-900 space-y-2 flex-shrink-0">
          <div className="flex items-center justify-between font-bold">
            <span className="flex items-center gap-1.5 text-slate-900">
              <ShieldCheck className="w-4 h-4 text-emerald-600" />
              <span>Safe Daily Focus: <strong className="text-slate-900">{draft.safeCapacityMinutes}m</strong></span>
            </span>
          ...
          <p className="text-slate-700 text-[12px] leading-relaxed font-medium">
            {draft.reasoning}
          </p>
          <div className="text-[11px] font-semibold text-slate-600">
            💡 Review the proposed horizon transfers below. Click <strong className="text-slate-900">[ ✕ ]</strong> on any task to override and keep it in Today.
          </div>
        </div>
        ...
        <div className="space-y-5 overflow-y-auto flex-1 pr-1.5 py-1">
          ...
          <div className="space-y-2.5">
            <div className="flex items-center justify-between">
              ...
              <span className="label-mono text-[11px] text-slate-600 font-bold">
                {tomorrowTasks.reduce((sum, t) => sum + getSanitizedTaskMinutes(t), 0)} mins
              </span>
            </div>

            {tomorrowTasks.length === 0 ? (
              <div className="p-3.5 rounded-xl border-2 border-dashed border-slate-300 text-center text-xs text-slate-500 font-medium bg-slate-50">
              ...
              <div className="space-y-2">
                {tomorrowTasks.map((t) => (
                  <div
                    key={t.id}
                    className="p-3 rounded-xl bg-white border-2 border-slate-900 shadow-[2px_2px_0px_#0f172a] flex items-center justify-between gap-3 text-xs"
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="w-6 h-6 rounded bg-slate-100 border border-slate-900 text-slate-900 flex items-center justify-center flex-shrink-0 font-mono font-black text-[10px]">
                        →
                      </div>
                      <div className="truncate">
                        ...
                        <span className="label-mono text-[10px] text-slate-500 font-bold">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg border-2 border-slate-900 bg-rose-100 hover:bg-rose-200 text-rose-900 transition-transform active:scale-95 cursor-pointer"
                      >
                        <X className="w-3.5 h-3.5 text-rose-900" strokeWidth={3} />
                      </button>
                    </div>
                  </div>
                ))}
            ...
            <div className="space-y-2.5 pt-2">
              ...
              <div className="space-y-2">
                {nextWeekTasks.map((t) => (
                  <div
                    key={t.id}
                    className="p-3 rounded-xl bg-purple-50 border-2 border-slate-900 shadow-[2px_2px_0px_#0f172a] flex items-center justify-between gap-3 text-xs"
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="w-6 h-6 rounded bg-purple-200 border border-slate-900 text-slate-900 flex items-center justify-center flex-shrink-0 font-mono font-black text-[10px]">
                        📅
                      </div>
                      <div className="truncate">
                        <span className="font-black text-slate-900 truncate block text-xs sm:text-sm">
                        ...
                        <span className="label-mono text-[10px] text-purple-700 font-bold">
          ...
          {totalProposedCount === 0 && (
            <div className="p-6 rounded-2xl bg-emerald-50 border-2 border-slate-900 text-center space-y-2">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-3 border-t-2 border-slate-900 flex-shrink-0">
          <div className="text-xs text-slate-600">
            ...
            (<span className="font-mono font-bold text-emerald-700">{totalFreedMinutes}m</span> freed)
            {overriddenCount > 0 && (
          ...
          <div className="flex items-center gap-3 self-end sm:self-auto">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 rounded-xl border-2 border-slate-900 bg-white hover:bg-slate-100 text-slate-900 font-bold text-xs transition-transform active:translate-x-0.5 active:translate-y-0.5 cursor-pointer shadow-[2px_2px_0px_#0f172a]"
            >
              Cancel
            </button>
            <button
